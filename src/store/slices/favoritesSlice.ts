...
const favoritesSlice = createSlice({
  ...
  initialState,
  reducers: {
    addItemToFavorites: (state, { payload }: PayloadAction<IBookDetails>) => {
      if (!state.item.find((book) => book.isbn13 === payload.isbn13)) {
        state.item.push(payload);
      } else {
        const index = state.item.findIndex((book) => book.isbn13 === payload.isbn13);
...
export const { addItemToFavorites } = favoritesSlice.actions;
